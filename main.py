...
@app.post("/add-todo")
async def add_todo(item: TodoItem = Body(...)): # أضفنا Body هنا
    try:
        # تأكد إن الـ task_text موجودة جوه الـ item
        text = item.task_text
        ...
        return {"status": "success", "data": data.data} # أضفنا .data هنا للأمان
    except Exception as e:
        print(f"Error: {e}") # عشان يظهرلك في لوجز ريل واي
